function[matlabbatch] = First_level_MS2_mentalRP_prm(matlabbatch, GLMprm,...
    subid, sub_idx, iRun, subj_analysis_folder)
%[matlabbatch] = First_level_MS2_mentalRP_prm(matlabbatch, GLMprm,...
%                       subid, sub_idx, iRun, subj_analysis_folder)
% First_level_MS2_mentalRP_prm loads the first level infos for
% First_level_MS2_megaconcatenation_NicoC_batch.m
%
% INPUTS
% matlabbatch: batch that needs to be filled with the info of the stroop task
%
...
stroop   = getfield( load([subj_analysis_folder, filesep, 'onsets_sub',subid,'_stroop_run',run_nm],'stroop'), 'stroop');
onset       = stroop.onset; % onsets
duration    = stroop.duration; % duration
mod         = stroop.mod; % modulators
...
dur_inc = stroopRPprm.dur_inc;
mod_inc = stroopRPprm.mod_inc;
switch o_inc
    case {1,2,3}
        switch o_inc 
            case 1 % all trials pooled
                curr_oInc_names = {'S_inc'};
                trialTypes_oInc = {'all'};
            case 2 % to gain/to lose trials separated
                curr_oInc_names = {'S_inc_toGain','S_inc_toLose'};
                trialTypes_oInc = {'toGain','toLose'};
            case 3
                curr_oInc_names = {'S_inc_noErrorTrials','S_inc_ErrorTrials'};
                trialTypes_oInc = {'all_noErrorTrials','all_ErrorTrials'};
        end
        ...
        for iCond_oInc = 1:length(curr_oInc_names)
            ...
            if mod_inc.RT_fp ~= 0 && ~ismember(mod_inc.RT_fp,[1])
                Nmod.(curr_oInc_name) = Nmod.(curr_oInc_name) + 1;
                idx_oInc_RTfp = Nmod.(curr_oInc_name);
                switch mod_inc.RT_fp
                    case 2
                        modName.(curr_oInc_name)(idx_oInc_RTfp) = {'RT first pair'};
                        modulators.(curr_oInc_name)(:,idx_oInc_RTfp) = mod.(trialType_oInc).RT_fp;
                    otherwise
                        error('not ready yet');
                end
            end
            ...
            if mod_inc.inc_bis ~= 0
                ...
                switch mod_inc.inc_bis
                    ...
                    case 3
                        modName.(curr_oInc_name)(idx_oInc_inc_bis) = {'incentive rank motivational value'};
            ...
            if mod_inc.ROI_activity_yn ~= 0
                ...
                modulators.(curr_oInc_name)(:,idx_oInc_ROI) = ROI_data.(ROI_nm).S.(ROI_period_nm).(jRun_nm).(sub_nm)(trialN_idx);
            end
            ...
            if mod_inc.n_pairs_solved ~= 0
                Nmod.(curr_oInc_name) = Nmod.(curr_oInc_name) + 1;
                idx_oInc_nPairsSolved = Nmod.(curr_oInc_name);
                switch mod_inc.n_pairs_solved
                    case 1
                        modName.(curr_oInc_name)(idx_oInc_nPairsSolved) = {'number of pairs solved'};
                        modulators.(curr_oInc_name)(:,idx_oInc_nPairsSolved) = mod.(trialType_oInc).n_pairs_solved;
                    otherwise
                        error('not ready yet');
                end
            end

            %% number of incongruent pairs solved/number of pairs solved
            if mod_inc.n_incong ~= 0
                Nmod.(curr_oInc_name) = Nmod.(curr_oInc_name) + 1;
                idx_oInc_incong = Nmod.(curr_oInc_name);
                switch mod_inc.n_incong
                    case 1
                        modName.(curr_oInc_name)(idx_oInc_incong) = {'n_incong_solved'};
                        modulators.(curr_oInc_name)(:,idx_oInc_incong) = mod.(trialType_oInc).n_incong;
                    case 2
                        modName.(curr_oInc_name)(idx_oInc_incong) = {'perc_incong_solved'};
                        modulators.(curr_oInc_name)(:,idx_oInc_incong) = mod.(trialType_oInc).perc_incong;
                    otherwise
                        error('not ready yet');
                end
            end
            ...
            if mod_inc.n_errors ~= 0
                if (o_inc == 3 &&...
                        strcmp(trialType_oInc,'all_ErrorTrials') &&...
                        mod_inc.n_errors ~= 1) || o_inc ~= 3
                    % no point for noErrorTrials or if variable is just binary (error/no-error trial) since trials already split along error presence
                    
                    Nmod.(curr_oInc_name) = Nmod.(curr_oInc_name) + 1;
                    idx_oInc_nErrors = Nmod.(curr_oInc_name);
                    switch mod_inc.n_errors
                        case 1
                            modName.(curr_oInc_name)(idx_oInc_nErrors) = {'trials with errors'};
                            modulators.(curr_oInc_name)(:,idx_oInc_nErrors) = mod.(trialType_oInc).n_errorTrials;
                        case 2
                            modName.(curr_oInc_name)(idx_oInc_nErrors) = {'number of errors per trial'};
                            modulators.(curr_oInc_name)(:,idx_oInc_nErrors) = mod.(trialType_oInc).n_errors_per_trial;
                        case 3
                            modName.(curr_oInc_name)(idx_oInc_nErrors) = {'percentage of errors per trial'};
                            modulators.(curr_oInc_name)(:,idx_oInc_nErrors) = mod.(trialType_oInc).perc_errors_per_trial;
                        otherwise
                            error('not ready yet');
                    end % n_errors
                end
...
%                 benefit_var    = loadStruct_mdl.benefit_var.(['model_',num2str(mod_inc.mdl_n)]);
            ...
            %% mean(RT) all but first pair
            if mod_inc.RT_mRT ~= 0
                Nmod.(curr_oInc_name) = Nmod.(curr_oInc_name) + 1;
                idx_oInc_RT_mRT = Nmod.(curr_oInc_name);
                switch mod_inc.RT_mRT
                    case 1
                        modName.(curr_oInc_name)(idx_oInc_RT_mRT) = {'mean RT next pairs'};
                        modulators.(curr_oInc_name)(:,idx_oInc_RT_mRT) = mod.(trialType_oInc).mRT_without_fPair;
                    case 2
                        modName.(curr_oInc_name)(idx_oInc_RT_mRT) = {'mean RT next pairs'};
                        modulators.(curr_oInc_name)(:,idx_oInc_RT_mRT) = mod.(trialType_oInc).mRT_without_fPair_without_Errors;
                    otherwise
                        error('not ready yet');
                end
            end
...
dur_dispE   = stroopRPprm.dur_dispE;
mod_dispE   = stroopRPprm.mod_dispE;
switch o_dispE
    case {1,2,3}
        switch o_dispE
            case 1 % all trials pooled
                curr_oEScale_names = {'S_effortScale'};
                trialTypes_oEScale = {'all'};
            case 2 % to gain/to lose trials separated
                curr_oEScale_names = {'S_effortScale_toGain','S_effortScale_toLose'};
                trialTypes_oEScale = {'toGain','toLose'};
            case 3 % no-error/error trials separated
                curr_oEScale_names = {'S_effortScale_noErrorTrials','S_effortScale_ErrorTrials'};
                trialTypes_oEScale = {'all_noErrorTrials','all_ErrorTrials'};
        end
        ...
        for iCond_oDispE = 1:length(curr_oEScale_names)
            ...
            switch dur_dispE
                ...
                case 2 % boxcar from effort scale to RT
                    durCond.(curr_oEScale_name) = duration.(trialType_oEScale).effortScale - duration.(trialType_oEScale).firstPress;
            end
            ...
            if mod_dispE.RT_fp ~= 0 && ~ismember(mod_dispE.RT_fp,[1])
                Nmod.(curr_oEScale_name) = Nmod.(curr_oEScale_name) + 1;
                idx_oDispE_RTfp = Nmod.(curr_oEScale_name);
                switch mod_dispE.RT_fp
                    case 2
                        modName.(curr_oEScale_name)(idx_oDispE_RTfp) = {'RT first pair'};
                        modulators.(curr_oEScale_name)(:,idx_oDispE_RTfp) = mod.(trialType_oEScale).RT_fp;
                    otherwise
                        error('not ready yet');
                end
            end
            ...
            if mod_dispE.inc ~= 0
                ...
                switch mod_dispE.inc
                    ...
                    case 3
                        modName.(curr_oEScale_name)(idx_oDispE_inc) = {'incentive rank motiv value'};
                        modulators.(curr_oEScale_name)(:,idx_oDispE_inc) = mod.(trialType_oEScale).absIncentiveRank;
                    case 4
                        modName.(curr_oEScale_name)(idx_oDispE_inc) = {'incentive motiv value'};
            ...
            if mod_dispE.inc_bis ~= 0
                ...
                switch mod_dispE.inc_bis
                    ...
                    case 3
                        modName.(curr_oEScale_name)(idx_oDispE_inc_bis) = {'incentive rank motivational value'};
            ...
            if mod_dispE.ROI_activity_yn ~= 0
                ...
                ROI_data = getfield(load([subj_analysis_folder,'ROI',filesep,'GLM',GLM_ROI_nm,'_',ROI_nm,'.mat'],...
                    'sub_ROI_trial_b_trial'),'sub_ROI_trial_b_trial');
                modulators.(curr_oEScale_name)(:,idx_oDispE_ROI) = ROI_data.(ROI_nm).S.(ROI_period_nm).(jRun_nm).(sub_nm)(trialN_idx);
            end
            ...
            if mod_dispE.n_pairs_solved ~= 0
                Nmod.(curr_oEScale_name) = Nmod.(curr_oEScale_name) + 1;
                idx_oDispE_nPairsSolved = Nmod.(curr_oEScale_name);
                switch mod_dispE.n_pairs_solved
                    case 1
                        modName.(curr_oEScale_name)(idx_oDispE_nPairsSolved) = {'number of pairs solved'};
                        modulators.(curr_oEScale_name)(:,idx_oDispE_nPairsSolved) = mod.(trialType_oEScale).n_pairs_solved;
                    otherwise
                        error('not ready yet');
                end
            end
            ...
            if mod_dispE.n_incong ~= 0
                Nmod.(curr_oEScale_name) = Nmod.(curr_oEScale_name) + 1;
                idx_oDispE_incong = Nmod.(curr_oEScale_name);
                switch mod_dispE.n_incong
                    case 1
                        modName.(curr_oEScale_name)(idx_oDispE_incong) = {'n_incong_solved'};
                        modulators.(curr_oEScale_name)(:,idx_oDispE_incong) = mod.(trialType_oEScale).n_incong;
                    case 2
                        modName.(curr_oEScale_name)(idx_oDispE_incong) = {'perc_incong_solved'};
                        modulators.(curr_oEScale_name)(:,idx_oDispE_incong) = mod.(trialType_oEScale).perc_incong;
                    otherwise
                        error('not ready yet');
                end
            end
            ...
            if mod_dispE.n_dist ~= 0
                Nmod.(curr_oEScale_name) = Nmod.(curr_oEScale_name) + 1;
                idx_oDispE_nDist = Nmod.(curr_oEScale_name);
                switch mod_dispE.n_dist
                    case 1
                        modName.(curr_oEScale_name)(idx_oDispE_nDist) = {'avg_dist_btw_numbers'};
                        modulators.(curr_oEScale_name)(:,idx_oDispE_nDist) = mod.(trialType_oEScale).m_nDist;
                    case 2
                        modName.(curr_oEScale_name)(idx_oDispE_nDist) = {'avg 1/distance between numbers'};
                        modulators.(curr_oEScale_name)(:,idx_oDispE_nDist) = mod.(trialType_oEScale).m_inverse_nDist;
                    otherwise
                        error('not ready yet');
                end
            end
            ...
            if mod_dispE.n_errors ~= 0
                if (o_dispE == 3 &&...
                        strcmp(trialType_oEScale,'all_ErrorTrials') &&...
                        mod_dispE.n_errors ~= 1) || o_dispE ~= 3
                    % no point for noErrorTrials or if variable is just binary (error/no-error trial) since trials already split along error presence
                    
                    Nmod.(curr_oEScale_name) = Nmod.(curr_oEScale_name) + 1;
                    idx_oDispE_nErrors = Nmod.(curr_oEScale_name);
                    switch mod_dispE.n_errors
                        case 1
                            modName.(curr_oEScale_name)(idx_oDispE_nErrors) = {'trials with errors'};
                            modulators.(curr_oEScale_name)(:,idx_oDispE_nErrors) = mod.(trialType_oEScale).n_errorTrials;
                        case 2
                            modName.(curr_oEScale_name)(idx_oDispE_nErrors) = {'number of errors per trial'};
                            modulators.(curr_oEScale_name)(:,idx_oDispE_nErrors) = mod.(trialType_oEScale).n_errors_per_trial;
                        case 3
                            modName.(curr_oEScale_name)(idx_oDispE_nErrors) = {'percentage of errors per trial'};
                            modulators.(curr_oEScale_name)(:,idx_oDispE_nErrors) = mod.(trialType_oEScale).perc_errors_per_trial;
                        otherwise
                            error('not ready yet');
                    end % n_errors
                end
...
%                 benefit_var = loadStruct_mdl.benefit_var.(['model_',num2str(mod_dispE.mdl_n)]);
            ...
            if mod_dispE.RT_mRT ~= 0
                Nmod.(curr_oEScale_name) = Nmod.(curr_oEScale_name) + 1;
                idx_oDispE_RT_mRT = Nmod.(curr_oEScale_name);
                switch mod_dispE.RT_mRT
                    case 1
                        modName.(curr_oEScale_name)(idx_oDispE_RT_mRT) = {'mean RT next pairs'};
                        modulators.(curr_oEScale_name)(:,idx_oDispE_RT_mRT) = mod.(trialType_oEScale).mRT_without_fPair;
                    case 2
                        modName.(curr_oEScale_name)(idx_oDispE_RT_mRT) = {'mean RT next pairs'};
                        modulators.(curr_oEScale_name)(:,idx_oDispE_RT_mRT) = mod.(trialType_oEScale).mRT_without_fPair_without_Errors;
                    otherwise
                        error('not ready yet');
                end
            end
...
%% onset effort performance (=start answering to the first pair)

%% onset feedback
o_fbk   = stroopRPprm.o_fbk;
dur_fbk = stroopRPprm.dur_fbk;
mod_fbk = stroopRPprm.mod_fbk;
switch o_fbk
    case {1,2,3}
        switch o_fbk
            case 1 % all trials pooled
                curr_oFbk_names = {'S_feedback'};
                trialTypes_oFbk = {'all'};
            case 2 % to gain/to lose trials separated
                curr_oFbk_names = {'S_feedback_toGain','S_feedback_toLose'};
                trialTypes_oFbk = {'toGain','toLose'};
            case 3 % no-error/error trials separated
                curr_oFbk_names = {'S_feedback_noErrorTrials','S_feedback_ErrorTrials'};
                trialTypes_oFbk = {'all_noErrorTrials','all_ErrorTrials'};
        end
        ...
        for iCond_oFbk = 1:length(curr_oFbk_names)
            ...
            if mod_fbk.ROI_activity_yn ~= 0
                ...
                ROI_data = getfield(load([subj_analysis_folder,'ROI',filesep,'GLM',GLM_ROI_nm,'_',ROI_nm,'.mat'],...
                    'sub_ROI_trial_b_trial'),'sub_ROI_trial_b_trial');
                modulators.(curr_oFbk_name)(:,idx_oFbk_ROI) = ROI_data.(ROI_nm).S.(ROI_period_nm).(jRun_nm).(sub_nm)(trialN_idx);
            end
    ...
    case 0 % nothing happens because not included in the GLM
    otherwise
        error(['case o_fbk = ',num2str(o_fbk),' not ready']);
end % o_fbk

%% onset cross
o_cross     = stroopRPprm.o_cross;
dur_cross   = stroopRPprm.dur_cross;
switch o_cross
    case 1 % onset for fixation cross across all trials and conditions
        n_cond = n_cond + 1;
        cond_nm = [cond_nm, 'S_cross'];
        onsetCond.S_cross = onset.ITI;
